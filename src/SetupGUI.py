...
class SetupGUI(QtWidgets.QDialog):
    ...
    def tableWidget_DrawList_slot(self, data, typ):
        ...
        self.tableWidget_DrawList.setHorizontalHeaderLabels(
            ["Number", "Object", "Coordinates"]
        )
        ...
        for n, key in enumerate(data.keys()):  # Row
            ...
            for m, item in enumerate(data[key]):
                self.tableWidget_DrawList.setItem(
                    r + n, m + 1, QtWidgets.QTableWidgetItem(str(item))
                )
    ...
    def save_config(self):
        ...
        with open(self.path_file_label, "w", encoding="utf-8") as f:
            ...
            QtWidgets.QMessageBox.information(
                self, "Notification", "Successfully saved!"
            )
    ...
    def browser_file(self):
        try:
            ...
            path_file, _ = QtWidgets.QFileDialog.getOpenFileName(
                ...
                "Choose template",
                "",
                "Images/Video (*.png *.jpg *.bmp *.mp4 *.avi *.mov)",
                options=options,
            )
            if path_file:
                ...
                if file_extension in [".png", ".jpg", ".bmp"]:
                    ...
                    if ret:
                        ...
                        QtWidgets.QMessageBox.critical(
                            self, "Error", "Incorrect video format"
                        )
    ...
    def confirm_draw(self):
        if pa.DRAW_COORDINATE:
            ...
            if int(pa.DRAW_COORDINATE["width"]) < 0:
                temp["x"] = int(pa.DRAW_COORDINATE["x"]) + int(
                    pa.DRAW_COORDINATE["width"]
                )
            ...
            if int(pa.DRAW_COORDINATE["height"]) < 0:
                temp["y"] = int(pa.DRAW_COORDINATE["y"]) + int(
                    pa.DRAW_COORDINATE["height"]
                )
    ...
    def remove_draw(self):
        ...
        if button:
            ...
            xy = json.loads(
                str(self.tableWidget_DrawList.item(row, 2).text()).replace("'", '"')
            )
    ...
    def fitView(self):
        self.view.fitInView(
            self.scene.sceneRect(), QtCore.Qt.KeepAspectRatioByExpanding
        )
