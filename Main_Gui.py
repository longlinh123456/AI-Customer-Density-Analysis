...
class MainGUI(QtWidgets.QMainWindow):
    ...
    def start(self):
        try:
            ...
            Timer.Timer(
                function=self.monitor_pc_performance,
                name="pc_performance",
                forever=True,
                interval=2,
                type="repeat",
    ...
    def open_video(self):
        try:
            ...
            if video_file:
                Timer.Timer(
                    function=self.Main.auto_video, name="auto_video", args=[video_file]
                ).start()
    ...
    def monitor_pc_performance(self):
        try:
            ...
            get_updater().call_latest(
                self.progressBar_RAM.setValue, int(mem_stats.percent)
            )
            get_updater().call_latest(
                self.progressBar_DISK.setValue, int(disk_stats.percent)
            )
            get_updater().call_latest(
                self.ram.setText,
